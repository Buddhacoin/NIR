...
| Prepare identities, sync candidate context, and sign admission offline (testnet Slices A/B1/B2a) | [`docs/validator-join.md`](docs/validator-join.md) |
...
| Independently compiled consensus cross-check | Rust reproduces canonical encoding, exact authorization-envelope binding, and ordinary, sponsored, Transfer Credit and multisignature monetary/resource transitions from shared language-neutral vectors; independent signature verification and full-node execution remain outside this profile |
...
| Paginated account history proofs | Protocol-v24 account state commits to the exact count and fixed-depth indexed Merkle root of every related transaction; redundant journals remain the recovery source while a versioned, checkpoint-verified SQLite index starts without full replay and serves bounded pages and transaction proofs without retaining the whole history in RAM |
| Multi-source history recovery | Post-quantum signed manifests, bounded hash-addressed chunks, manifest-first HTTPS agreement, disk-staged streaming with source fallback, crash-resumable activation, and operator commands are implemented locally; independent public deployment remains |
| Exclusive node writer | The node command atomically locks its data directory, rejects a second live writer, safely recovers a dead process lock, and fails closed on malformed ownership data |
...
| Open validator candidate queue | Protocol v31 locks the exact minimum bond, requires current-quorum live endpoint/TLS/transport observation, delays eligibility, and prevents rotations from skipping the FIFO head; protocol v32 additionally binds both admission signatures to the exact genesis and a 64-block provisional authorization lifetime; active-set size/timing remain quorum-governed and automatic churn is not yet implemented |
...
6. For contribution roles and the intended mining workflow, read
   ...
   and [`docs/mining.md`](docs/mining.md). For wallet and transfer commands, read
...
- `blockchain/validator-admission.mjs` — protocol-v31 admission/readiness and deterministic queue records, plus the protocol-v32 chain- and height-bound admission envelope.
...
- `blockchain/account-history-database.mjs` — rebuildable disk-native transaction locator, account pages, and Merkle proof nodes.
...
- `blockchain/archive-service.mjs` — immutable manifest/chunk HTTP service and bounded multi-source HTTPS client.
...
- `docs/validator-admission.md` — bounded v31 candidate queue, v32 admission lifetime, live-readiness certificate, migration, and honest remaining limits.
