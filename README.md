...
| Paginated account history proofs | Protocol-v24 account state commits to the exact count and fixed-depth indexed Merkle root of every related transaction; redundant journals remain the recovery source while a rebuildable SQLite index serves bounded pages and transaction proofs without retaining the whole history in RAM |
