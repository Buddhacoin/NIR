...
An assignment-chain-anchor v4 proof must carry an offline-verifiable checkpoint
trust package. The verifier accepts no caller-supplied checkpoint validator list:
it obtains the v5 proof and validator set only from that package and checks them
against an externally pinned witness-policy ID, minimum package sequence, and
minimum checkpoint height. The floors are mandatory integers and must come from
rollback-protected local trust state. The verifier checks prepare and commit
quorums, requires the header's validator-set commitment to match the packaged set,
...
current validator set. An operator must independently pin the witness-policy ID
and durably advance its anti-replay floors. A state snapshot alone is not
sufficient. A foreign history with the same network name, an arbitrary
...
The mandatory production package described in
