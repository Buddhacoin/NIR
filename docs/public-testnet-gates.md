...
| 14. Launch review | Re-run gates 1–13 at the agreed observation time, then `npm run launch-review -- create`, `sign`, `verify` | Published hash index, evidence packages, limitations and contact list | A canonical package binds one result and artifact hash for every gate, the exact network and observation window, and requires every configured operator/reviewer signature | Independent security reviewers and all operators sign the launch decision; unresolved critical findings keep the gate closed |
